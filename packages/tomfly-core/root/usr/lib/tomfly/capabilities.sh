#!/bin/sh
# TomFly - kernel protocol capability matrix.
#
...
TOMFLY_CAP_MIHOMO="vless vmess trojan ss hysteria2 hy2 tuic anytls"
TOMFLY_CAP_SINGBOX="vless vmess trojan ss hysteria2 hy2 tuic anytls"

# tomfly_kernel_supports <kernel> <type> -> returns 0 if the protocol is supported
tomfly_kernel_supports() {
    local list
    case "$1" in
        singbox) list=" ${TOMFLY_CAP_SINGBOX} " ;;
        *)       list=" ${TOMFLY_CAP_MIHOMO} " ;;
    esac
    case "$list" in
        *" $2 "*) return 0 ;;
        *)        return 1 ;;
...
# tomfly_node_incompat <kernel> <uci-section>
#
...
tomfly_node_incompat() {
    local kernel="$1" sec="$2" type security
    type=$(uci -q get "tomfly.${sec}.type" 2>/dev/null)
    security=$(uci -q get "tomfly.${sec}.security" 2>/dev/null)

    [ -z "$type" ] && { echo "missing protocol type"; return 1; }

    if ! tomfly_kernel_supports "$kernel" "$type"; then
