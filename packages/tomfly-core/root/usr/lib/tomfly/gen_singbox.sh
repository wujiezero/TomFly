...
generate_singbox_config() {
    ...
    printf '    {"type": "tun", "tag": "tun-in", "interface_name": "TomFly", "inet4_address": "172.19.0.1/30", "auto_route": true, "auto_redirect": true, "strict_route": false, "stack": "system"}\n'
    printf '  ],\n'
    printf '  "outbounds": [\n'
    ...
    uci show tomfly 2>/dev/null | grep "^tomfly\.proxy_.*\.type=" | while read -r line; do
        ...
        local reason
        reason=$(tomfly_node_incompat singbox "$sec") || { log_warn "skipping node $(_u "${sec}.name"): $reason"; continue; }
        [ "$first" = "0" ] && printf ','
        printf '\n'
